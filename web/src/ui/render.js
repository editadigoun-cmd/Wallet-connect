...
function fillProviders(id,items,empty){
  ...
  if(!grid)return;
  grid.innerHTML=items.length?items.map((p,i)=>'<button type="button" class="provider-card '+(i===0?"selected":"")+'" data-provider="'+escapeHtml(p.correspondent)+'"><span class="provider-logo">'+(p.logo?'<img src="'+escapeHtml(p.logo)+'" alt="">':escapeHtml(providerInitials(p)))+'</span><span><strong>'+escapeHtml(providerLabel(p))+'</strong><small>'+escapeHtml(p.currency||state.meData?.currency||"")+' · Disponible</small></span></button>').join(""):'<div class="provider-empty">'+escapeHtml(empty)+'</div>';
