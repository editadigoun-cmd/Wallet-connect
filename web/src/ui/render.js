...
function fillProviders(id,items,empty){const s=document.getElementById(id);if(!s)return;s.innerHTML=items.length?'<option value="">Sélectionner un moyen</option>'+items.map(p=>'<option value="'+escapeHtml(p.correspondent)+'">'+providerInitials(p)+" · "+escapeHtml(providerLabel(p))+" · "+escapeHtml(p.currency||state.meData?.currency||"")+'</option>').join(""):'<option value="">'+escapeHtml(empty)+'</option>'}
