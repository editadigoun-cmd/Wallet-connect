...
export function initProfile({api,toast,render,show}){
  document.getElementById("saveProfileBtn").addEventListener("click",async()=>{
    try{
      ...
      const d=await api("/profile",{method:"POST",body:JSON.stringify({country_code:country,full_name,phone})});
      state.meData={...state.meData,...d.user};
      state.providers=await api("/providers?country="+encodeURIComponent(country));
